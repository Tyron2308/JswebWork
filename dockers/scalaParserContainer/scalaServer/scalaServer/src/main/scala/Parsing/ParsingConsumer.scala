...
class ParsingConsumerActor extends Actor {
  ...
  private def emptyBatch(batch: Array[((String, String), Int)], msize: Long, count :Int): Boolean = {
    ...
    if (count <= msize) {
      ...
      return emptyBatch(newBatch, msize,
        workers.size * ParsingConsumerActor.detailBAtch)
    }
    true
  }
  ...
  override def receive: Receive = {
    ...
    case LaunchWorker(batch, number) => {
      ...
      emptyBatch(b, number, ParsingConsumerActor.detailBAtch)
      self ! Done
    }
    ...
    case Done =>
      println(Console.GREEN + "Un batch a ete traite, count = "
        + mcount + " worker size "
        + workers.size + "-----"
        + " --- save sizze ="
        + save.size)
